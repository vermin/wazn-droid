...
import io.wazn.androidminer.api.ChangelogItem;
...
import io.wazn.androidminer.controls.SimpleTriangleIndicator;
        ...
        implements BottomNavigationView.OnNavigationItemSelectedListener
{
    ...
    private void updatePayoutWidget(ProviderData d) {
        if(d.isNew) {
            ...
            sBalance = sBalance.replace("WAZN", "").trim();
    ...
    public void enablePayoutWidget(boolean enable, String text) {
        ...
        if (enable) {
            ...
            TextView tvWAZNUnit = findViewById(R.id.waznunit);
            tvWAZNUnit.setVisibility(View.VISIBLE);
            ...
            TextView tvWAZNUnit = findViewById(R.id.waznunit);
            tvWAZNUnit.setVisibility(View.INVISIBLE);
    ...
    private void showAboutHaku() {
        ...
        tvWebsite.setOnClickListener(new View.OnClickListener() {
            public void onClick(View v) {
                Uri uri = Uri.parse(getResources().getString(R.string.WaznLinkClean));
                startActivity(new Intent(Intent.ACTION_VIEW, uri));
            }
        });
    ...
    private void updateHashrateTicks(float fHr, float fMaxHr) {
        ...
        if((meterTicks.getTickNumber() == 0 || (fCurrentMax > 0 && fHr >= fCurrentMax * 0.9)) && fMaxHr > 0) {
            ...
            // Sometimes waznrig returns a wrong low hrMax, but it needs to be > tick number, so we force a dummy max number
            if(hrMax <= 10.0f) {
                hrMax = 15.0f;
            }
    ...
    private void saveBitmap(Bitmap bitmap) {
        ...
        imagePath = new File(contextOfApplication.getExternalFilesDir(Environment.DIRECTORY_PICTURES), "wazn_miner_screenshot.png");
